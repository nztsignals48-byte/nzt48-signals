...
# W12 Advanced Self-Learning (Crammer 2006, Page-Hinkley, Gelman 2013, Dietterich 2000, Settles 2009)
try:
    ...
    _W12_AVAILABLE = False
    import logging as _l; _l.getLogger(__name__).warning("W12 modules not available: %s", _e)
...
class RegimePerformanceMatrix:
    ...
    def get_confidence_adjustment(self, regime: str, strategy: str, direction: str) -> int:
        ...
        # Harvey & Liu (2015): regime-conditional WR needs ≥15 observations to be statistically
        # distinguishable from chance at 95% confidence. Below 15, variance is too high.
        if not cell or cell.trades < 15:
            return 0  # Not enough data for reliable estimate
    ...
    def should_disable_strategy(self, regime: str, strategy: str, direction: str) -> bool:
        ...
        Disable if: 25+ trades AND win rate < 30% AND avg R < -0.5
        Harvey & Liu (2015): need ≥25 trades for statistically reliable regime-
        conditional disable decision at 95% confidence (raised from 10).
        """
        cell = self.matrix.get(regime, {}).get(strategy, {}).get(direction)
        if not cell or cell.trades < 25:
            return False
...
class LearningEngine:
    ...
    def __init__(self):
        ...
        # W12 Advanced Self-Learning Engine
        self.incremental_learner = IncrementalLearner() if _W12_AVAILABLE else None
        self.drift_detector = DriftDetector() if _W12_AVAILABLE else None
        self.bayesian_win_rate = BayesianWinRate() if _W12_AVAILABLE else None
        self.ensemble_diversity = EnsembleDiversitySystem() if _W12_AVAILABLE else None
        self.active_weighter = ActiveLearningWeighter() if _W12_AVAILABLE else None
        self._w12_trade_count = 0  # W12 trade counter for batch triggers
    ...
    def record_trade(self, trade: Trade) -> None:
        ...
        # --- W12 Advanced Self-Learning (per-trade updates) ---
        if _W12_AVAILABLE:
            try:
                # Build outcome dict for W12 modules
                _outcome_dict = {
                    "ticker": trade.ticker,
                    "strategy": trade.strategy,
                    "status": "WIN" if trade.pnl_r_multiple > 0 else "LOSS",
                    "r_multiple": trade.pnl_r_multiple,
                    "confidence": trade.confidence_score,
                    "regime": trade.regime_state,
                    "rvol": getattr(trade, "rvol_at_entry", 1.5),
                    "adx": getattr(trade, "adx", 20),
                    "vix": getattr(trade, "vix_at_entry", 18),
                    "sector_rank": getattr(trade, "sector_rank", 3),
                    "momentum_score": getattr(trade, "momentum_score", 0.5),
                }
                self._w12_trade_count += 1

                # 1. IncrementalLearner: O(1) per-trade PA update (Crammer et al. 2006)
                if self.incremental_learner:
                    self.incremental_learner.update(_outcome_dict)

                # 2. DriftDetector: Page-Hinkley test for regime shifts
                if self.drift_detector:
                    _win = trade.pnl_r_multiple > 0
                    _drift = self.drift_detector.update(_win)
                    if _drift:
                        self.logger.warning(
                            "W12 DRIFT DETECTED after trade #%d — regime shift likely, "
                            "applying exponential forgetting to old outcomes",
                            self._w12_trade_count
                        )

                # 3. BayesianWinRate: update per-regime win rate estimate (every 5 trades)
                if self.bayesian_win_rate and self._w12_trade_count % 5 == 0:
                    _bwr_regime = trade.regime_state or "UNKNOWN"
                    self.bayesian_win_rate.update(
                        wins=1 if trade.pnl_r_multiple > 0 else 0,
                        losses=0 if trade.pnl_r_multiple > 0 else 1,
                        key=f"S15:{_bwr_regime}",
                        regime=_bwr_regime,
                    )

                # 4. ActiveLearningWeighter: flag high-learning-value trades
                if self.active_weighter:
                    _lv = self.active_weighter.get_learning_value(_outcome_dict)
                    if _lv > 0.7:
                        self.logger.debug(
                            "W12 HIGH-VALUE TRADE: %s %s conf=%d R=%.2f learning_value=%.2f",
                            trade.ticker, trade.strategy, trade.confidence_score,
                            trade.pnl_r_multiple, _lv
                        )

                # 5. EnsembleDiversity retrain: every 100 trades
                if self.ensemble_diversity and self._w12_trade_count % 100 == 0:
                    try:
                        import json
                        from pathlib import Path
                        _outcomes_path = Path("data/outcomes.jsonl")
                        if _outcomes_path.exists():
                            _all_outcomes = []
                            with open(_outcomes_path) as _f:
                                for _line in _f:
                                    if _line.strip():
                                        try:
                                            _all_outcomes.append(json.loads(_line))
                                        except Exception:
                                            pass
                            if len(_all_outcomes) >= 50:
                                _weights = None
                                if self.active_weighter and self.drift_detector:
                                    _weights = self.active_weighter.get_learning_weights(
                                        _all_outcomes,
                                        drift_weights=self.drift_detector.get_sample_weights(_all_outcomes)
                                    )
                                _result = self.ensemble_diversity.train_all(_all_outcomes, sample_weights=_weights)
                                self.logger.info(
                                    "W12 ENSEMBLE RETRAINED: accuracy=%.3f diversity=%s n=%d",
                                    _result.get("ensemble_accuracy", 0),
                                    _result.get("diversity_summary", ""),
                                    len(_all_outcomes)
                                )
                    except Exception as _ed:
                        self.logger.debug("W12 ensemble retrain skipped: %s", _ed)

            except Exception as _w12_err:
                self.logger.debug("W12 learning cycle error: %s", _w12_err)
    ...
    def get_w12_telegram_summary(self) -> str:
        """Returns W12 self-learning engine status for nightly Telegram digest."""
        if not _W12_AVAILABLE:
            return "W12 Learning: modules not available"
        lines = ["🧠 <b>SELF-LEARNING ENGINE (W12)</b>"]
        # PA incremental learner
        if self.incremental_learner:
            ready = self.incremental_learner.is_ready()
            lines.append(f"  PA Learner: {'Active' if ready else 'Warming up'} (per-trade O(1) updates)")
        # Drift detector
        if self.drift_detector:
            ds = self.drift_detector.get_status()
            drift_count = ds.get("total_drifts_detected", 0)
            lines.append(f"  Drift Detector: {drift_count} regime shifts detected (Page-Hinkley)")
        # Bayesian win rate
        if self.bayesian_win_rate:
            estimates = self.bayesian_win_rate.get_all_estimates()
            if estimates:
                best_k = max(estimates, key=lambda k: estimates[k].get("posterior_mean", 0))
                best_wr = estimates[best_k].get("posterior_mean", 0)
                lines.append(f"  Bayesian WR: Best regime={best_k} WR={best_wr:.1%} (beta-binomial CI)")
        # Ensemble diversity
        if self.ensemble_diversity:
            last_acc = getattr(self.ensemble_diversity, "_last_accuracy", None)
            if last_acc:
                lines.append(f"  Ensemble: Last accuracy={last_acc:.3f} (LGB+XGB+PA stacked)")
        lines.append(f"  Trades processed: {self._w12_trade_count}")
        return "\n".join(lines)
    ...
    def get_all_learning_status(self) -> dict:
        ...
        status = {
            ...
            # W12 Advanced Self-Learning status
            "w12_learning": {
                "available": _W12_AVAILABLE,
                "trade_count": self._w12_trade_count,
                "incremental_ready": self.incremental_learner.is_ready() if self.incremental_learner else False,
                "drift_status": self.drift_detector.get_status() if self.drift_detector else {},
                "bayesian_estimates": self.bayesian_win_rate.get_all_estimates() if self.bayesian_win_rate else {},
                "ensemble_accuracy": getattr(self.ensemble_diversity, "_last_accuracy", None) if self.ensemble_diversity else None,
            },
        }
