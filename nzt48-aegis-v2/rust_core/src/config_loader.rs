...
impl EngineConfig {
    ...
    pub fn load_live(config_dir: &Path) -> Result<Self, ConfigError> {
        ...
        if let Some(trading) = live.trading
            && let Some(v) = trading.daily_trade_limit { cfg.risk.daily_trade_limit = v; }
        ...
        if let Some(timing) = live.timing
            && let Some(v) = timing.stale_data_threshold_secs { cfg.risk.stale_data_threshold_secs = v; }
        ...
        if let Some(hardening) = live.hardening {
            ...
            if let Some(sizing) = hardening.sizing
                && let Some(v) = sizing.min_trade_gbp_live { cfg.hardening.sizing.min_trade_gbp_live = v; }
        }
    ...
    fn load_universe(path: &Path) -> Result<Vec<RawTicker>, ConfigError> {
        ...
        if watchlist_path.exists()
            && let Ok(tickers) = Self::load_universe_from_watchlist(&watchlist_path)
            && !tickers.is_empty() {
                eprintln!(
    ...
    fn load_contracts(path: &Path) -> Result<Vec<ContractEntry>, ConfigError> {
        ...
        for contract in &mut contracts {
            if contract.leverage < 1 || contract.leverage > 10 {
                ...
                contract.leverage = contract.leverage.clamp(1, 10);
            }
