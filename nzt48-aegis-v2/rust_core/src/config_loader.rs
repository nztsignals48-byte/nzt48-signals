...
impl EngineConfig {
    ...
    pub fn load(config_dir: &Path) -> Result<Self, ConfigError> {
        ...
        let risk = RiskConfig {
            ...
            entry_cutoff_secs: 15 * 3600 + 45 * 60,
...
mod tests {
    ...
    fn test_load_full_config() {
        ...
        assert_eq!(cfg.risk.confidence_floor, 65.0);
        assert_eq!(cfg.risk.daily_drawdown_pct, 2.0);
        ...
        assert_eq!(cfg.crucible.max_positions_override, 1);
    }
    ...
    fn test_contracts_loaded() {
        ...
        assert_eq!(cfg.contracts.len(), 92);
        let qqq3 = cfg
            .contracts
            .iter()
    ...
    fn test_universe_loaded() {
        let config_dir = Path::new(env!("CARGO_MANIFEST_DIR"))
            ...
            .join("config");
        let cfg = EngineConfig::load(&config_dir).expect("load config");
        assert!(cfg.tickers.len() >= 12); // At least 12 core tickers
        let qqq3 = cfg
            .tickers
            .iter()
            .find(|t| t.symbol == "QQQ3.L")
            .expect("QQQ3.L");
        assert_eq!(qqq3.leverage, 3);
    }
