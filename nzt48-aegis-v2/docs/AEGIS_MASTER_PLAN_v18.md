# AEGIS V2 — MASTER PLAN v18
### AEGIS Adaptive Global Execution & Intelligence System
**Version**: 18.0 | **Date**: 2026-03-09 | **Status**: APPROVED — IMPLEMENTATION READY

> This document is the canonical master plan. It supersedes v17. It synthesises the full V1 codebase audit, V2 Rust engine audit, Phase 11-13 specifications, Gemini adversarial audit (200 bullets + binding mandates), and Claude self-analysis triage. All findings are triaged and injected into their respective phases. Phases are broken into granular, independently verifiable units (max ~20h each).
...
| Telegram webhook + PyMuPDF PDFs | Phase 17 |
...
Gemini produced three rounds of analysis: (1) 200-bullet adversarial audit + top-10 priority fixes, (2) binding architectural mandates (Polars, crossbeam, IBKR pacing), (3) fatal flaw corrections (pacing paradox, scanner conservation, tier nomenclature). All triaged below.
...
| G-04 | Corporate action spin-offs can void ISA tax wrapper | HIGH | Phase 12 — corporate action veto |
...
| G-10 | Telegram polling: 2s HALT latency during flash crashes | MEDIUM | Phase 17 — async webhook |
...
| **GEM-A2** | **Lock-free ring buffer.** `crossbeam-channel` bounded (capacity=1000) between Rust ticks and Python bridge. `TrySendError::Full` → drop oldest tick. | Phase 8 |
...
**Phase 8:** SC-09 crossbeam buffer, SC-10 cost-basis tracker, SC-11 AtomicUsize line counter, SC-04 two-tier data architecture (IBKR active-only + external bulk EOD)
...
**Phase 14:** Chandelier floor ≥ 1.5× bid_ask_spread; TWAP U-shaped (60-day median volume curve, not flat); partial fill dust < £500 → immediate market order

**Phase 15:** CVaR uses Cornish-Fisher expansion (not normal); DCC-GARCH veto computed async, cached 5-min TTL

**Phase 16:** Polars mandate; remove log AUM taper; Yellow tier escalation on Ouroboros failure; 500-ticker batch processing

**Phase 17:** Webhook replaces polling; Rust-side HALT priority interrupt; heartbeat every 30 min; 4000-char message truncation
...
**Hours**: 32h | **Status**: NEXT — must complete before Phase 11

**Rationale**: SC-01 to SC-11 are blocking items. None of Phases 11+ can proceed without them. Phase 8 also injects the 3 Gemini binding mandates (GEM-A2 crossbeam, GEM-A3 data tier, SC-10/11 precision fixes).

**Deliverables:**

| SC | Item | File | Gemini Ref |
|----|------|------|-----------|
...
| SC-06 | Dust guard < £500 in exit_engine.rs; partial fill remainder < £500 → immediate market order | exit_engine.rs | — |
...
| SC-09 | `crossbeam-channel` bounded ring buffer (capacity=1000) between Rust ticks and Python bridge; `TrySendError::Full` → drop oldest, log overflow counter | python_bridge.rs | GEM-A2 |
...
**Gate**: All 11 items coded + unit tested; `cargo test` passes; `docker build` passes; crossbeam buffer stress-tested at 10,000 ticks/sec synthetic load
...
**Rationale**: Routing logic determines which instrument we trade — critical to ETP-first principle, ISA compliance, and corporate action safety.

**Deliverables:**

- `smart_router.rs` (NEW):
  - ETP-first principle: ETP wins unless no ETP exists OR (direct_cost < etp_cost × 0.9 AND health passes)
  - Full cost comparison: FX drag + FTT + IBKR commission + stamp duty
...
- **Corporate Action Veto (Gemini G-04):** `reqContractDetails` corporate action check in RiskGate G1:
  - Check `tradingHours` + IBKR corporate action calendar fields on universe scan
  - Hard-veto any ticker with spin-off, merger, or special dividend within 48h of ex-date
  - Prevents non-ISA-eligible spin-off landing in ISA wrapper (would void tax status)
  - `CorporateActionVeto { ticker, action_type, ex_date, hours_until_ex }`
...
- AT-28: Corporate action veto fires for spin-off in 24h
- AT-29: Corporate action veto clears 24h after ex-date passes
- AT-30: Cost comparison includes all 4 components (FX + FTT + commission + stamp duty)

**Gate**: 14 tests pass; FTT gate verified both France + Italy thresholds; ISA triple-gate verified; corporate action veto verified with synthetic ex-date in 24h
...
**Rationale**: The exit and execution engine. Gemini found 3 critical flaws — Chandelier floor whipsaw, flat TWAP, dust after partial fills.
...
- `executioner_v2.rs` (NEW):
  ...
  - Partial fill FSM: remainder < £500 after cancel → immediate market order (no dust)
...
- AT-56: Partial fill £400 remainder → immediate market order (not cancelled)
- AT-57: Partial fill £600 remainder → limit order placed (not market)
...
**Gate**: 17 tests pass; ratchet proptest 1000 cases; Chandelier floor verified at low-ATR high-spread condition; TWAP U-shape distribution test
...
- `risk_arbiter.rs` EXTENDED — 9 new veto checks added to existing 22:
  ...
  6. `DustGuard` — position < £500 after partial fill
...
**Rationale**: The nightly intelligence pipeline. Gemini mandates: Polars or OOM kill, no log AUM taper, Yellow tier on failure. Pacing paradox requires external data source for universe screening.

**Deliverables:**

- `ouroboros/` EXTENDED — 9-step pipeline:
...
- AT-94: Polygon.io fallback to Databento if HTTP 429 response

**Gate**: 16 tests pass; Polars run verified ≤ 3.5GB RSS on 1000-ticker test; checkpoint resume from step 5 verified; Yellow escalation + Telegram alert end-to-end verified
...
**Rationale**: Operational visibility. Gemini mandate: webhook replaces polling (2s HALT latency unacceptable). Heartbeat prevents silent death.
...
- **Webhook architecture (Gemini G-10):** `python-telegram-bot` async webhook. Rust-side priority interrupt channel: HALT commands received via webhook → write to `mpsc::Sender<HaltCommand>` → engine reads within 1 event loop tick. HALT fires within 100ms.
...
- AT-99: HALT command via Telegram → engine receives within 100ms
...
**Gate**: 15 tests pass; manually verify Telegram message arrives in channel within 100ms for HALT; both PDFs open in viewer; heartbeat verified over 2h with missed heartbeat simulation
...
5. **Suite 5 — ISA Compliance Audit**
   ...
   - Verify: corporate action veto fires for synthetic spin-off ticker
...
| **8** | Pre-Conditions + P0 + Gemini mandates (SC-01→SC-11) | 32h | **NEXT** | Unit tests per SC item |
| **11** | 5-Mode Clock + SubscriptionManager + Conservation Rule | 20h | NOT STARTED | AT-01→16 |
| **12** | Smart Router + ISA Gate + Corporate Action Veto | 15h | NOT STARTED | AT-17→30 |
| **13** | HotScanner + RotationScanner (5s time-EWMA OFI) | 20h | NOT STARTED | AT-31→47 |
| **14** | Infinite Chandelier (floor fix) + Executioner V2 (U-TWAP) | 20h | NOT STARTED | AT-48→64 |
| **15** | RiskGate 31 Vetoes + CVaR Cornish-Fisher | 15h | NOT STARTED | AT-65→78 |
| **16** | Ouroboros + Polars + Yellow escalation + External EOD | 20h | NOT STARTED | AT-79→94 |
| **17** | Telemetry Stack (webhook + heartbeat + PDFs) | 15h | NOT STARTED | AT-95→109 |
...
| **TOTAL REMAINING** | | **310h** | | **AT-01→AT-194 = 194 tests** |

**At 20h/week**: ~15.5 weeks to live capital
**At 40h/week**: ~7.75 weeks to live capital

*310h reflects 296h baseline + 14h Gemini mandates absorbed (SC-09/10/11 +6h, Polars rewrite +5h, webhook+heartbeat +3h)*
...
Paste this into a new Claude Code terminal session to begin implementation:

```
Begin Phase 8 of AEGIS_MASTER_PLAN_v18.md. Reference file:
/Users/rr/nzt48-signals/nzt48-aegis-v2/docs/AEGIS_MASTER_PLAN_v18.md

Implement all 11 SC items in order. Write unit tests for each. Run cargo test after each SC item before proceeding to the next.
...
SC-06: Dust guard in exit_engine.rs — if partial fill remainder < £500 GBP → submit market order immediately, do not leave dust
...
SC-09: crossbeam-channel bounded ring buffer (capacity=1000) in python_bridge.rs replacing blocking subprocess call per tick — Rust writes asynchronously via try_send; dedicated Python reader thread pulls batches; TrySendError::Full → drop oldest tick, increment overflow counter logged to WAL
...
After all 11 items have passing tests:
...
*AEGIS_MASTER_PLAN_v18.md — Generated 2026-03-09*
*Supersedes: AEGIS_MASTER_PLAN_v17.md*
...
*Gemini adversarial audit: 200 bullets triaged; 5 binding mandates injected; 3 fatal flaws fixed*
